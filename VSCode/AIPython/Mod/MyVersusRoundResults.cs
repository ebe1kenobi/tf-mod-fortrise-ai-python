  ...
  public class MyVersusRoundResults
  {
    ...
    public static void Update_patch(On.TowerFall.VersusRoundResults.orig_Update orig, global::TowerFall.VersusRoundResults self)
    {
      ...
      for (var i = 0; i < self.Components.Count; i++)
      {
        ...
        if (!LoaderAIImport.CurrentPlayerIs("HUMAN", playerIndex))
        {
          //dynData.Set("text", LoaderAIImport.GetPlayerTypePlaying(playerIndex) + " " + (playerIndex + 1));
          dynData.Set("text", LoaderAIImport.GetPlayerName(playerIndex));
          text.Position.X -= 30;
        }
